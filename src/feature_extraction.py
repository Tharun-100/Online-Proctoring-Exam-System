# creating  a feature extrating module for fraud detection system
import cv2
import numpy as np
...
class FeatureExtrator:
    def __init__(self):
        """Initialize Mediapipe and yolo models for feature extraction"""
        self.base_face_det = python.BaseOptions(model_asset_path=r"E:\Projects in ML\FRAUD DETECTION SYSTEM FOR THE ONLINE PROCTORED EXAMS\mediapipe_models\blaze_face_short_range.tflite")

        self.face_det_options = vision.FaceDetectorOptions(
            base_options=self.base_face_det,
            min_detection_confidence=0.5
        )
        
        self.face_detector = vision.FaceDetector.create_from_options(self.face_det_options)

        self.base_face_lm = python.BaseOptions(
        ...
        self.face_lm_options = vision.FaceLandmarkerOptions(
            base_options=self.base_face_lm,
            output_face_blendshapes=False,
            output_facial_transformation_matrixes=True,
            num_faces=1
        )
        self.face_landmarker = vision.FaceLandmarker.create_from_options(self.face_lm_options)

        self.base_hand = python.BaseOptions(
        ...
        self.hand_options = vision.HandLandmarkerOptions(
            base_options=self.base_hand,
            num_hands=2
        )
        self.hand_landmarker = vision.HandLandmarker.create_from_options(self.hand_options)
        self.phone_model = YOLO(r"E:\Projects in ML\FRAUD DETECTION SYSTEM FOR THE ONLINE PROCTORED EXAMS\Testing\yolov8n.pt")  # COCO class 67 = cellphone
        self.FEATURE_ORDER = ["face_present","no_of_face","face_x","face_y","face_w","face_h","left_eye_x","left_eye_y","right_eye_x","right_eye_y","nose_tip_x","nose_tip_y","mouth_x","mouth_y","face_conf","hand_count","left_hand_x","left_hand_y","right_hand_x","right_hand_y","hand_obj_interaction","head_pose","head_pitch","head_yaw","head_roll","phone_present","phone_loc_x","phone_loc_y","phone_conf","gaze_on_script","gaze_direction","gazePoint_x","gazePoint_y","pupil_left_x","pupil_left_y","pupil_right_x","pupil_right_y"]
    ...
    def extract_features(image_path):
        ...
        feats = {k: 0 for k in self.FEATURE_ORDER}

        # -------- FACE DETECTION --------
        face_det = self.face_detector.detect(mp_img)
        ...
        # -------- FACE LANDMARKS --------
        face_lm = self.face_landmarker.detect(mp_img)
        ...
        # -------- HAND LANDMARKS --------
        hand_res = self.hand_landmarker.detect(mp_img)
        ...
        # -------- PHONE DETECTION --------
        yolo_res = self.phone_model(img, verbose=False)[0]
