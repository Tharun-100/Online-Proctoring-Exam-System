...
def _extract_windows_from_video(video_path: str,*,fps_sample: Optional[float] = None,max_seconds: Optional[float] = None,) -> Tuple[List[Dict[str, Any]], float]:
    ...
    if current_window_rows:
        # Use the actual last timestamp instead of blindly closing the window at
        # current_window_end. This prevents a 10-sec video from becoming 11
        # windows, or a 2-sec video from becoming 3 windows.
        actual_end_sec = float(current_window_rows[-1].get("timestamp_sec", current_window_start) or current_window_start)
        actual_duration = actual_end_sec - float(current_window_start)

        # Keep the final partial window only if it contains at least half a second
        # of evidence. Small leftovers are too noisy for duration-based decisions.
        if actual_duration >= 0.5 * WINDOW_SECONDS:
            feats = _create_window_features(current_window_rows)
            window_rows.append(
                {
                    "window_start_sec": float(current_window_start),
                    "window_end_sec": float(actual_end_sec),
                    **feats,
                }
            )
...
def analyze_video() -> Any:
    ...
    # Keep API output compact by default. Send full_response=true only when debugging.
    full_response_raw = (
        request.form.get("full_response")
        or request.args.get("full_response")
        or "false"
    )
    full_response = str(full_response_raw).strip().lower() in {"1", "true", "yes", "y"}
    ...
    try:
        ...
        compact_timeline = [
            {
                "start_sec": round(float(item["window_start_sec"]), 2),
                "end_sec": round(float(item["window_end_sec"]), 2),
                "event": item["dominant_event_key"],
                "event_label": item["dominant_event"],
                "ratio": round(float(item["dominant_ratio"]), 3),
                "probability": round(float(item["xgb_probability"]), 4),
            }
            for item in timeline
            if item["dominant_event_key"] != "normal"
        ]

        event_summary = {
            s["event_key"]: {
                "label": s["type"],
                "triggered": bool(s["is_triggered"]),
                "total_seconds": round(float(s["total_seconds"]), 2),
                "longest_streak_seconds": round(float(s["longest_streak_seconds"]), 2),
                "max_ratio": round(float(s["max_ratio"]), 3),
            }
            for s in problem_summary
        }
        ...
        response_payload = {
            ...
            "max_suspicious_probability": round(float(max_w.probability), 4),
            "decision_threshold": decision_threshold,
            "exact_problem": exact_problem,
            "event_summary": event_summary,
            "event_timeline": compact_timeline,
        }
